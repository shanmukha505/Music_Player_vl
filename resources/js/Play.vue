<template>
    <div class="name" id="play">
        <div class="audio" align="center" style="">
            ...
            <div class="is-center">
                ...
                <button
                    ...
                    v-on:click="play"
                    title="play"
                    :class="{ active: !active, pause: active }"
                    type="button"
                ></button>
            ...
            <div class="is-center">
                <button
                    ...
                    title="suffle"
                    v-on:click="suffle"
                    :class="{ suffle: !disabled, suffles: disabled }"
                    type="button"
                ></button>
                ...
                <button
                    ...
                    title="repeat"
                    type="button"
                    :class="{ repeat: !sdisabled, repeats: sdisabled }"
                    v-on:click="repeats"
                ></button>
                ...
                <button
                    ...
                    title="single repeat"
                    type="button"
                    :class="{ single: ssingle, singles: ssingle }"
                    v-on:click="single"
                ></button>
            </div>
            ...
            <button
                ...
                v-on:click="some_toggle = !some_toggle"
            >
                Albums
            </button>
        </div>
        ...
        <div v-for="albums in album" :key="albums">
            <album :album="albums" v-if="some_toggle"></album>
        </div>
    </div>
</template>
    ...
    mounted() {
        ...
        axios.post("/song").then(response => {
            ...
            if (this.user == "") {
                ...
                for (var i = 0; i < this.songs.length; i++) {
                    if (
                        this.songs[i]["visible"] == "private" &&
                        this.songs[i]["user_id"] != this.user
                    ) {
                        this.songs.splice(i, 1);
    ...
    methods: {
        ...
        changed() {
            ...
            if (secs.toString().length == 1)
                this.currenttime =
                    this.mins + ":0" + (Math.round(this.sliderValue) % 60);
            else
                this.currenttime =
                    this.mins + ":" + (Math.round(this.sliderValue) % 60);
        },
        ...
        async readFromBlob(blob, name, index) {
            ...
            musicMetadata.fetchFromUrl(src).then(musicMetadata => {
                ...
                var image =
                    ...
                    this.meta.common.picture[0].data.toString("base64");
                ...
                if (this.album.length == 0) {
                    ...
                    for (var i = 0; i < this.album.length; i++) {
                        if (this.album[i].album == this.meta.common.album) {
                            ...
                            this.album[i].user_id.push(
                                this.songs[index]["user_id"]
                            );
        ...
        play() {
            ...
            if (this.player == false) {
                ...
                if (this.previoussong != this.name) {
                    ...
                    if (this.ssingle == true) this.audio.stop();
        ...
        repeat() {
            ...
            else this.duration = mins + ":" + Math.round(secs);
            ...
            if (
                Math.round(this.audio.seek()) ==
                Math.round(this.audio.duration()) - 1
            ) {
                this.mins = 0;
        ...
        view(name) {
            ...
            for (var i = 0; i < this.songs.length; i++) {
                if (this.songs[i]["name"] == name) this.index = i;
            }
